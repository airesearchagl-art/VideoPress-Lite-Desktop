...
起動時に同梱FFmpegの `ffmpeg -encoders` を解析し、以下のNVENCエンコーダを検出します。
...
`nvidia-smi` で RTX 4090 を検出し、かつ `h264_nvenc` が利用可能な場合は、GPU(H264 NVENC)をデフォルト選択します。GPU選択時はCPUの `libx264` ではなく、対応するNVENCエンコーダで圧縮します。
